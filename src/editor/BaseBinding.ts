	...
	private findOpenBase(): InternalBaseView | null {
		...
		const inspect = (leaf: any) => {
			...
			if (isInternalBaseView(view)) found = view;
				...
				console.warn("[Realtime] Obsidian Bases view API shape is unsupported; using disk write-through fallback.");
			}
		};
...
function isInternalBaseView(value: unknown): value is InternalBaseView {
	...
	return !!view && typeof view.getViewData === "function" && typeof view.setViewData === "function" && typeof view.requestSave === "function";
}
