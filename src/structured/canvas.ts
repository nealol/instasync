...
export function serializeCanvas(value: JsonValue): string {
	...
	const nodeOrder = Array.isArray(root.nodeOrder) ? root.nodeOrder.filter(isString) : Object.keys(nodes);
	const edgeOrder = Array.isArray(root.edgeOrder) ? root.edgeOrder.filter(isString) : Object.keys(edges);
