...
const DEFAULT_TEXT_KEYS = new Set(["text", "label", "filter", "formula", "query", "name", "description"]);

function shouldUseText(key: string | undefined, value: unknown, options?: ReconcileOptions): boolean {
	if (typeof value !== "string") return false;
	const keys = options?.textKeys ?? DEFAULT_TEXT_KEYS;
	return !!key && (keys.has(key) || value.length > 256);
}

export function reconcileInto(parent: Y.Map<any> | Y.Array<any>, value: JsonValue, options: ReconcileOptions = {}): void {
...
function reconcileMap(map: Y.Map<any>, value: Record<string, JsonValue>, options: ReconcileOptions): void {
	for (const key of Array.from(map.keys())) {
		if (!(key in value)) map.delete(key);
	}
	for (const [key, next] of Object.entries(value)) {
		const current = map.get(key);
		const reconciled = reconcileValue(current, next, key, options, (created) => map.set(key, created));
