...
function TimelineView({ plugin, initialHash }: { plugin: RealtimePlugin; initialHash?: string }) {
	...
	return (
		<div className="realtime-timeline-shell">
			...
			{isAdmin && selected && (
				<RollbackBar plugin={plugin} vaultId={vaultId} hash={selected} onDone={() => void loadPage()} />
			)}
		</div>
	);
}
...
function CommitDetailPane({
	...
	const changedPaths = useMemo(
		() => changes.map((c) => c.renamedTo ?? c.path),
		[changes],
	);
	...
	return (
		<div className="realtime-timeline-rollback">
			<span className="setting-item-description">
				Restore the whole vault to this commit. History is preserved — the restore is applied
				as a new change.
			</span>
