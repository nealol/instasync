...
function TrashView({ plugin }: { plugin: RealtimePlugin }) {
	...
	return <>
		<h3>Trash</h3>
		<p className="setting-item-description">Deleted files are kept here and can be restored to their original
			location or a new path. Permanently deleting an attachment reclaims its storage.</p>
		<input className="realtime-modal-input" type="text" placeholder="Filter by path…" value={query}
			   onChange={(event) => setQuery(event.currentTarget.value)} />
		{filtered.length === 0
			? <p className="setting-item-description">{entries.length === 0 ? "Trash is empty." : "No matches."}</p>
			: <div className="realtime-trash-list">
				{filtered.map((entry) => <TrashRow key={entry.id} plugin={plugin} entry={entry} />)}
			</div>}
	</>;
}
...
function TrashRow({ plugin, entry }: { plugin: RealtimePlugin; entry: TrashEntry }) {
	...
	const restore = (path?: string) => void (async () => {
		setBusy(true);
		try {
	...
	return <div className="realtime-trash-row">
		<div className="realtime-trash-info">
			<div className="realtime-trash-path">{entry.path}</div>
			<div className="setting-item-description">{KIND_LABELS[entry.kind] ?? entry.kind} · deleted {formatWhen(entry.deletedAt)}</div>
			{renaming ? <div className="realtime-actions realtime-trash-rename">
				<input className="realtime-modal-input" type="text" value={target}
					   onChange={(event) => setTarget(event.currentTarget.value)} placeholder="New path" />
				<button className="mod-cta" disabled={busy || !target.trim()} onClick={() => restore(target)}>Restore here</button>
