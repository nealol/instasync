...
async function setup(vaultName: string): Promise<{ plugin: FakePlugin; cursors: RealtimeCursorsAPI; vaultId: string }> {
...
describe("remote cursors plugin API", () => {
	it("performs audited, cursor-attributed note edits via plain local calls", async () => {
		...
		expect(after.entries.map((e) => e.operation).slice(0, 2)).toEqual([
			"note_delete",
			"note_move",
		]);
