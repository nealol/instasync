...
const snapEntry = (kind: FileKind, hash: string): SyncFileState => ({ kind, hash, size: 1, mtimeMs: 1 });
...
describe("classifyStatus", () => {
	...
	it("flags both sides for a conflict candidate", () => {
		...
		expect(entries).toEqual([{ path: "a.png", kind: "attachment", local: "modified", remote: "modified" }]);
	});
});
