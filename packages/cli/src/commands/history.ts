...
export function registerHistoryCommands(program: Command): void {
	...
	history
		...
		.action(async (relPath: string | undefined, opts: { limit: number; before?: string }) => {
			...
			const page = await vault.history.listCommits({ limit: opts.limit, before: opts.before, path: relPath });
			out(ctx, page, () => {
				printTable(
					page.commits.map((c) => [
						c.shortHash,
		...
		.action(async (hash: string, relPath: string | undefined) => {
			...
			if (relPath) {
				...
				out(ctx, file, () => {
					...
					else if (file.type === "binary") process.stdout.write(`binary: ${file.hash} (${file.size} bytes)\n`);
			...
			out(ctx, detail, () => {
				...
				process.stdout.write(`author ${c.authorName} <${c.authorEmail}> ${formatTime(c.timestampMs)}\n`);
				if (c.cursorName) process.stdout.write(`cursor ${c.cursorName}\n`);
				process.stdout.write(`\n  ${c.subject}\n\n`);
				printTable(detail.changes.map((ch) => [ch.status, ch.renamedTo ? `${ch.path} → ${ch.renamedTo}` : ch.path]));
			});
		});
