...
import { CliError, findRtmdDir, writeRtmd, type AuthConfig, type RtmdConfig, type Workspace } from "../config";
...
function reportSync(ctx: Ctx, report: SyncReport, direction: "pull" | "push"): void {
	out(ctx, report, () => {
		...
		process.stdout.write(`${report.applied.length} ${noun}, ${report.conflicts.length} conflict(s)\n`);
	});
...
function ensureUnbound(dir: string): void {
	...
	if (existing) {
		throw new CliError(`${dir} is already inside an rtmd vault folder (${existing}); run this somewhere else`);
	}
}
...
export function registerWorkspaceCommands(program: Command): void {
	program
		...
		.action(async (dirArg: string | undefined, opts: { name?: string; baseUrl?: string; paste?: boolean }) => {
			...
			const config: RtmdConfig = { version: 1, baseUrl: session.baseUrl, vaultId: vault.id, vaultName: vault.name, auth };
			writeRtmd(dir, config);
			process.stderr.write(`Created vault "${vault.name}" (${vault.id}); pushing folder contents…\n`);
	...
	program
		...
		.option("--base-url <url>", "server origin")
		.option("--paste", "paste a session token instead of using the browser")
		.option("--cursor-token <token>", "authenticate as a remote cursor (vault must be an id; no browser login)")
		.option("--cursor-oauth <mcpUrl>", "OAuth-delegate a remote cursor by its MCP URL (vault must be an id)")
		.action(
			async (
				vaultArg: string,
				...
				if (opts.cursorToken || opts.cursorOauth) {
					...
					if (opts.cursorToken) {
						...
						auth = { mode: "cursor-oauth", clientId: session.clientId, tokens: liveToStoredTokens(session.tokens) };
					}
				} else {
