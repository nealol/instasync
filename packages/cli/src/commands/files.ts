...
export function registerFileCommands(program: Command): void {
  ...
  program
    ...
    .action(async (from: string, to: string) => {
      ...
      if (isExcluded(from) || isExcluded(to))
        throw new CliError("cannot move excluded (dot) paths");
  ...
  program
    ...
    .action(async (relPath: string, opts: { file?: string }) => {
      ...
      const note = exists
        ? await vault.notes.replace(relPath, content)
        : await vault.notes.create(relPath, content);
  ...
  program
    ...
    .action(async (relPath: string, opts: { old: string; new: string; all?: boolean }) => {
      ...
      const { ws, vault } = vaultClients(ctx);
      assertNotePath(relPath);
      const note = await vault.notes.patch(relPath, {
        old: opts.old,
        new: opts.new,
        replaceAll: opts.all,
      });
