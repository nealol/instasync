...
export function registerAuthCommands(program: Command): void {
	program
		...
		.action(async (dirArg: string | undefined, opts: { baseUrl?: string; paste?: boolean; vault?: string }) => {
			...
			const session = await loginUser({ baseUrl: opts.baseUrl ?? existing?.baseUrl, paste: opts.paste });
	...
	program
		...
		.action(async () => {
			...
			if (auth.mode === "user") {
				await requireUserClient(ws).logout().catch(() => {});
			} else {
				process.stderr.write(
		...
		.action(async () => {
			...
			if (auth?.mode === "user") {
				...
				out(ctx, { ...base, cursorName }, () => {
					process.stdout.write(`remote cursor${cursorName ? ` "${cursorName}"` : ""} (${auth.mode} auth)\n`);
					process.stdout.write(`vault: ${base.vaultName ?? base.vaultId} @ ${base.baseUrl}\n`);
				});
			} else {
				out(ctx, base, () => {
					process.stdout.write(`not logged in (vault binding: ${base.vaultName ?? base.vaultId} @ ${base.baseUrl})\n`);
				});
			}
		});
}
