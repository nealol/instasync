...
export function registerCursorCommands(program: Command): void {
	const cursor = program.command("cursor").description("manage remote cursors (user login required)");
	...
	cursor
		...
		.action(async () => {
			...
			out(ctx, cursors, () =>
				printTable(
					cursors.map((c) => [c.id, c.name, c.pluginId ? `plugin:${c.pluginId}` : "admin", formatTime(c.createdAt)]),
	...
	cursor
		...
		.action(async (id: string) => {
			...
			out(ctx, res, () => {
				process.stdout.write(`new secret token (shown once, the old one is dead):\n${res.secretToken}\n`);
			});
		});
		...
		.action(async (id: string, opts: { limit: number; before?: number }) => {
			...
			out(ctx, page, () => {
				...
				if (page.hasMore && page.entries.length > 0) {
					process.stdout.write(`…more: --before ${page.entries[page.entries.length - 1].createdAt}\n`);
				}
			});
		});
