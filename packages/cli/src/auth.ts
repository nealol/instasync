...
export async function loginUser(opts: LoginOptions): Promise<UserSession> {
  const baseUrl = (
    opts.baseUrl ?? (await ask("Server URL (e.g. https://realtime.example.com)"))
  ).replace(/\/+$/, "");
...
export async function chooseVault(client: RealtimeClient, vaultArg?: string): Promise<VaultInfo> {
  ...
  if (vaultArg) {
    const match = vaults.find((v) => v.id === vaultArg) ?? vaults.find((v) => v.name === vaultArg);
    if (!match)
      throw new CliError(`no vault named or with id "${vaultArg}" (you have ${vaults.length})`);
    return match;
  }
  if (vaults.length === 0) {
    throw new CliError(
      "you have no vaults; create one with `rtmd init` or `rtmd vault create <name>`",
    );
  }
  return pick(
    "Choose a vault",
    vaults,
    (v) => `${v.name} (${v.role}${v.owner ? ", owner" : ""}) ${v.id}`,
  );
}
