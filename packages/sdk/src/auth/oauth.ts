...
export class OAuthClient {
	...
	async authorizeUrl(opts: AuthorizeUrlOptions): Promise<{ url: string; verifier: string; state: string }> {
		const meta = await this.discover();
	...
	refresh(opts: { refreshToken: string; clientId: string; clientSecret?: string }): Promise<OAuthTokens> {
		return this.tokenRequest({
...
export class OAuthTokenProvider implements TokenProvider {
	...
	constructor(opts: { oauth: OAuthClient; clientId: string; clientSecret?: string; tokens: OAuthTokens }) {
