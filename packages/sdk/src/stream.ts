...
export class NoteStream {
	...
	static async open(opts: NoteStreamOptions): Promise<NoteStream> {
		...
		const started = await new Promise<ServerFrame>((resolve, reject) => {
			ws.addEventListener("open", () => {
				ws.send(JSON.stringify({ type: "start", path: opts.path, anchor: opts.anchor ?? { mode: "append" } }));
			});
			ws.addEventListener("message", (event: { data: unknown }) => {
				const frame = parseFrame(event.data);
				if (!frame) return;
				if (frame.type === "started") resolve(frame);
				else if (frame.type === "error") reject(new StreamError(frame.code ?? "error", frame.message ?? "stream error"));
			});
			ws.addEventListener("error", () => reject(new Error("WebSocket connection failed")));
			ws.addEventListener("close", (event: { code?: number; reason?: string }) => {
				reject(new Error(`WebSocket closed before start: ${event.code ?? ""} ${event.reason ?? ""}`.trim()));
			});
		});
	...
	async write(text: string): Promise<void> {
		...
		if (this.sentBytes + bytes > MAX_STREAM_BYTES) {
			throw new StreamError("too_large", `write would exceed the ${MAX_STREAM_BYTES}-byte session limit`);
		}
...
function parseFrame(data: unknown): ServerFrame | null {
	if (typeof data !== "string") {
		...
		else if (ArrayBuffer.isView(data as ArrayBufferView)) data = new TextDecoder().decode(data as Uint8Array);
