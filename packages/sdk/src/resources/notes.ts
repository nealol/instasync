...
export class NotesResource {
	...
	create(path: string, content = ""): Promise<Note> {
		return this.http.request("POST", `/api/vaults/${this.vaultId}/notes`, { body: { path, content } });
	}
	...
	permalink(path: string): Promise<PermalinkResponse> {
		return this.http.request("POST", `/api/vaults/${this.vaultId}/note-permalinks/${encodePath(path)}`);
	}
}
...
export class FrontmatterResource {
	...
	parse(path: string): Promise<FrontmatterResponse> {
		return this.http.request("GET", `/api/vaults/${this.vaultId}/note-frontmatter/${encodePath(path)}`);
	}

	/** Set/unset frontmatter keys; returns the updated note. */
	patch(path: string, edit: PatchFrontmatterBody): Promise<Note> {
		return this.http.request("PATCH", `/api/vaults/${this.vaultId}/note-frontmatter/${encodePath(path)}`, {
...
export class PeriodicNotesResource {
	...
	getOrCreate(period: PeriodicPeriod, opts: { date?: string; content?: string } = {}): Promise<Note> {
