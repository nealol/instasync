...
export class CanvasesResource {
	...
	create(path: string, value: unknown = {}): Promise<StructuredResponse> {
		return this.http.request("POST", `/api/vaults/${this.vaultId}/canvases`, { body: { path, value } });
	}
	...
	move(path: string, toPath: string): Promise<StructuredResponse> {
		return this.http.request("POST", `/api/vaults/${this.vaultId}/canvas-moves/${encodePath(path)}`, {
			body: { toPath },
		});
	}

	addNode(path: string, node: CanvasNodeBody): Promise<StructuredResponse> {
		return this.http.request("POST", `/api/vaults/${this.vaultId}/canvas-nodes/${encodePath(path)}`, {
			body: node,
		});
	}

	updateNode(path: string, id: string, patch: Record<string, unknown>): Promise<StructuredResponse> {
		return this.http.request("PATCH", `/api/vaults/${this.vaultId}/canvas-nodes/${encodePath(path)}`, {
			body: { id, ...patch },
		});
	}

	deleteNode(path: string, id: string): Promise<StructuredResponse> {
		return this.http.request("DELETE", `/api/vaults/${this.vaultId}/canvas-nodes/${encodePath(path)}`, {
			body: { id },
		});
	}

	addEdge(path: string, edge: CanvasEdgeBody): Promise<StructuredResponse> {
		return this.http.request("POST", `/api/vaults/${this.vaultId}/canvas-edges/${encodePath(path)}`, {
			body: edge,
		});
	}

	updateEdge(path: string, id: string, patch: Record<string, unknown>): Promise<StructuredResponse> {
		return this.http.request("PATCH", `/api/vaults/${this.vaultId}/canvas-edges/${encodePath(path)}`, {
			body: { id, ...patch },
		});
	}

	deleteEdge(path: string, id: string): Promise<StructuredResponse> {
		return this.http.request("DELETE", `/api/vaults/${this.vaultId}/canvas-edges/${encodePath(path)}`, {
			body: { id },
		});
	}
}
...
export class BasesResource {
	...
	create(path: string, value: unknown = {}): Promise<StructuredResponse> {
		return this.http.request("POST", `/api/vaults/${this.vaultId}/bases`, { body: { path, value } });
	}
	...
	updateView(path: string, name: string, patch: Record<string, unknown>): Promise<StructuredResponse> {
		return this.http.request("PATCH", `/api/vaults/${this.vaultId}/base-views/${encodePath(path)}`, {
			body: { name, ...patch },
		});
	}

	deleteView(path: string, name: string): Promise<StructuredResponse> {
		return this.http.request("DELETE", `/api/vaults/${this.vaultId}/base-views/${encodePath(path)}`, {
			body: { name },
		});
	}

	setFilters(path: string, value: unknown): Promise<StructuredResponse> {
		return this.http.request("PUT", `/api/vaults/${this.vaultId}/base-filters/${encodePath(path)}`, {
			body: { value },
		});
	}

	setViewFilters(path: string, viewName: string, value: unknown): Promise<StructuredResponse> {
		return this.http.request("PUT", `/api/vaults/${this.vaultId}/base-view-filters/${encodePath(path)}`, {
			body: { name: viewName, value },
		});
	}

	setFormula(path: string, name: string, value: unknown): Promise<StructuredResponse> {
		return this.http.request("PUT", `/api/vaults/${this.vaultId}/base-formulas/${encodePath(path)}`, {
			body: { name, value },
		});
	}

	deleteFormula(path: string, name: string): Promise<StructuredResponse> {
		return this.http.request("DELETE", `/api/vaults/${this.vaultId}/base-formulas/${encodePath(path)}`, {
			body: { name },
		});
	}

	setProperty(path: string, name: string, value: unknown): Promise<StructuredResponse> {
		return this.http.request("PUT", `/api/vaults/${this.vaultId}/base-properties/${encodePath(path)}`, {
			body: { name, value },
		});
	}

	deleteProperty(path: string, name: string): Promise<StructuredResponse> {
		return this.http.request("DELETE", `/api/vaults/${this.vaultId}/base-properties/${encodePath(path)}`, {
			body: { name },
		});
	}
}
