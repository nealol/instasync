...
import { Http, encodePath, normalizeBaseUrl, staticToken, type TokenProvider } from "../../src/http";
...
describe("Http", () => {
	...
	it("serializes query params and skips undefined", async () => {
		...
		await http.request("GET", "/api/vaults/v1/search", { query: { q: "a b", limit: 5, before: undefined } });
	});
	...
	it("retries once via onUnauthorized and succeeds", async () => {
		...
		const fetch = mockFetch((_url, init) => {
			...
			return auth === "Bearer fresh" ? jsonResponse(200, { ok: true }) : jsonResponse(401, { error: "unauthorized" });
		});
...
describe("RealtimeClient URL construction", () => {
	function capture() {
		...
		return { requests, client: new RealtimeClient({ baseUrl: "https://x.test", token: "t", fetch }) };
	}
