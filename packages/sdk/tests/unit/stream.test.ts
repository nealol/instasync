...
describe("NoteStream", () => {
	...
	it("rejects open() when the server errors before started", async () => {
		...
		await expect(NoteStream.open({ url: server.url, token: "t", path: "missing.md" })).rejects.toThrow(
			"no such note",
		);
	});
	...
	it("propagates mid-stream error frames to write()", async () => {
		let texts = 0;
		server.onFrame = (frame, ws) => {
			if (frame.type === "start") ws.send(JSON.stringify({ type: "started", guid: "g", position: 0 }));
			if (frame.type === "text" && ++texts === 1) {
				ws.send(JSON.stringify({ type: "error", code: "too_large", message: "session byte limit exceeded" }));
			}
		};
		const stream = await NoteStream.open({ url: server.url, token: "t", path: "a.md" });
	...
	it("applies ack backpressure: writes pause until the server acks", async () => {
		...
		for (const ws of pendingAcks.splice(0)) ws.send(JSON.stringify({ type: "ack", applied: ackSizes[0] }));
		await second;
